...
export default function AddRestaurantForm(props) {
  ...
  const addRestaurant = () => {
    if (!restaurantName || !restaurantAddress || !restaurantDescription || !restaurantPhone || !restaurantEmail) {
      ...
      toastRef.current.show("El negocio tiene que tener almenos una foto");
    } else if (!locationRestaurant) {
      toastRef.current.show("Tienes que localizar tu negocio en el mapa");
    } else {
      setIsLoading(true);
        ...
        db.collection("negocios")
          ...
          .catch(error => {
            ...
            toastRef.current.show(
              "Error al subir el negocio, intento más tarde"
            );
          });
      });
    }
